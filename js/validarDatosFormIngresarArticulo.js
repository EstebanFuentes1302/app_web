...
const formIngresarArticulo = document.getElementById('formIngresarArticulo');
...
const validarformIngresarArticulo = (e) => {
    switch(e.target.name){
        case "txtIngresar":
        ...
        $.ajax({
            ...
            success: function(response){
                ...
                if(JSON.parse(response) != 'null'){
                    ...
                    const formIngresarArticuloInputs = document.querySelectorAll('#formIngresarArticulo input');
                    //console.log(formIngresarArticuloInputs);
                    formIngresarArticuloInputs.forEach((input)=>{
                        input.addEventListener('keyup', validarformIngresarArticulo);
                        input.addEventListener('blur', validarformIngresarArticulo);
                    })
                }else{
                    Swal.fire({
                    title: 'Error',
...
$('#formIngresarArticulo').submit(function(e){
    e.preventDefault();
    ...
    if(camposModificar['cantidad']){
        $.ajax({
            url: '../controlador/CtrlIngresarArticulo.php',
            type: 'POST',
            ...
            success: function(response){
                ...
                if(JSON.parse(response)=='true'){
                    ...
                    Swal.fire({
                    title: 'Error',
                    text: 'No se puede ingresar esa cantidad',
                    icon: 'error',
                    background: '#121212',
                    color: 'white'
                    })
